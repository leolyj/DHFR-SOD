  ...
  for dataset in test_datasets:
    ...
    for data_index, temp1 in tqdm_iter:
      ...
      if semantic_pred is not None:
        ...
        semantic_pred_saliency = semantic_pred.cpu().detach().squeeze()
        ...
        # common zone as fg bg, other as unknown
        semantic_gen = torch.ones_like(semantic_pred_crf) # unknown
        semantic_gen[(semantic_pred_saliency > 0.5) * (semantic_pred_crf > 0.5)] = 0.333333 # fg
        semantic_gen[(semantic_pred_saliency <= 0.5) * (semantic_pred_crf > 0.5)] = 0.666666  # crf fg, sal bg
        semantic_gen[(semantic_pred_saliency <= 0.5) * (semantic_pred_crf <= 0.5)] = 0 # bg
        ...
        pred_pil = to_pil(semantic_gen).resize((ori_h, ori_w), resample=Image.NEAREST)
        pred_pil.save(gray_pred_name)
        pred_data = np.array(pred_pil).astype('float')
        pred_data = np.round(pred_data / 255 * 3)
