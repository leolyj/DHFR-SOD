...
def main():
    ...
    for dataset in test_datasets:
        ...
        for data_index, temp1 in tqdm_iter:
          ...
          if semantic_pred is not None:
            semantic_pred = (semantic_pred.view(resize_image_h, resize_image_w)
                                          .cpu()
                                          .detach())
            semantic_pred_name = os.path.join(save_path, base_name)
            pred_pil = to_pil(semantic_pred).resize((resize_image_h, resize_image_w), resample=Image.NEAREST)
            pred_pil.save(semantic_pred_name)

            # groundtruth resize
            mask_pil = gt_resize(Image.fromarray(semantic_batch['semantic_label']))
            ...
            max_pred_array = semantic_pred.max()
            min_pred_array = semantic_pred.min()
            if max_pred_array == min_pred_array:
                semantic_pred = semantic_pred / 255
            else:
                semantic_pred = (semantic_pred - min_pred_array) / (max_pred_array - min_pred_array)
            cal_total_metrics.update(semantic_pred, mask_array)
        ...
        msg = f" ==>> 在'{dataset}'测试集上结果\n >> {fixed_pre_results}"
        print(msg)
        log_path = os.path.join(global_args.save_dir, 'test_scrore.txt')
        make_log(log_path, msg)
