...
def construct_path_dict(proj_root, exp_name):
    ...
    timeStr = str(str(datetime.now()).split('.')[0]).replace(' ', '_').replace(':', '_')
    tr_log_path = os.path.join(pth_log_path, f"tr_{timeStr}.txt")
    te_log_path = os.path.join(pth_log_path, f"te_{timeStr}.txt")
    cfg_log_path = os.path.join(pth_log_path, f"cfg_{timeStr}.txt")
    trainer_log_path = os.path.join(pth_log_path, f"trainer_{timeStr}.txt")
