...
from spml.models.heads.spp import PSPP
...
class ResnetPspnet(ResnetBase):
  ...
  def __init__(self, backbone_depth, strides, dilations, config):
    ...
    self.pspp = nn.Sequential(
        PSPP(2048, 512, bn=True, relu=True),
        nn.Conv2d(512, dim, 1, 1, 0, 1, bias=True))
...
def resnet_101_pspnet(config):
  """PSPNet with resnet101 backbone.
  """
  return ResnetPspnet([3, 4, 23, 3], [1, 2, 1, 1], [1, 1, 2, 4], config)


def resnet_50_pspnet(config):
  """PSPNet with resnet50 backbone.
  """
  return ResnetPspnet([3, 4, 6, 3], [1, 2, 1, 1], [1, 1, 2, 4], config)

if __name__ == '__main__':
  model = resnet_101_pspnet(config).cuda()
  model.eval()
  img = {}
  img['image'] = torch.rand(8, 3, 352, 352).cuda()
  for i in range(100):
    out = model(img)
