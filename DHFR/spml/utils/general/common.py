...
def add_margin(embeddings, prototypes, margin=0.5, eps=1e-12):
  """Normalizes embedding by L2 norm.
  ...
  emb_norm = torch.norm(embeddings, dim=-1, keepdim=True)
  proto_norm = torch.norm(prototypes, dim=-1, keepdim=True)
  norm_matrix = torch.mm(emb_norm, proto_norm.t())
  distance_matrix = torch.mm(embeddings, prototypes.t())
  output = distance_matrix - norm_matrix * margin
  return output
