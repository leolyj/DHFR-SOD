...
def _calculate_log_likelihood(embeddings,
  ...
  similarities_add_margin = common_utils.add_margin(embeddings, prototypes, margin=0.5).mul_(concentration).exp_()
  sim_martix = torch.mm(embeddings, prototypes.t())
  similarities = sim_martix.mul_(concentration).exp_()
  ...
  pixel_to_prototype_similarities = torch.gather(
      similarities_add_margin, 1, instance_labels)
  ...
  if group_mode == 'segsort+':
    ...
    same_semantic_similarities = torch.sum(
        similarities_add_margin * same_semantic_array, 1, keepdim=True)
