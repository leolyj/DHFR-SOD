...
class ListTagDataset(ListDataset):
  ...
  def _get_datas_by_index(self, idx):
    ...
    if depth is not None:
        ...
        return image, depth, semantic_label, instance_label, semantic_tags
    else:
        if semantic_label is not None:
          cats = np.unique(semantic_label)
          semantic_tags = np.zeros((256, ), dtype=np.uint8)
  ...
  def _training_preprocess(self, idx):
    ...
    if self.depth_paths is not None:
      image, depth, semantic_label, instance_label, semantic_tags = self._get_datas_by_index(idx)

      image = Image.fromarray(image.astype('uint8'))
      depth = Image.fromarray(depth.astype('uint8'))

      # sematic bg, sal_bg_crf_fg, sal_fg_crf_bg, fg: 0 2 3 1 -> 0 85 170 255
      semantic_label_mask = semantic_label.copy()
      semantic_label[semantic_label_mask == 2] = 85  # sal_bg_crf_fg
      semantic_label[semantic_label_mask == 3] = 170 # sal_fg_crf_bg
      ...
      # augmentation
      image,semantic_label,instance_label,depth = transforms.cv_random_flip(image,semantic_label,instance_label,depth)
      image,semantic_label,instance_label,depth = transforms.randomCrop(image, semantic_label,instance_label,depth)
      image,semantic_label,instance_label,depth = transforms.randomRotation(image, semantic_label,instance_label,depth)
      ...
      # 0 255/4 255*2/4 255*3/4 255 -> 0 1 2
      semantic_label_ori = semantic_label.copy()
      semantic_label[semantic_label_ori >= 3 / 4] = 1 # fg
      semantic_label[(semantic_label_ori >= 2 / 4) & (semantic_label_ori < 3 / 4)] = 3  # sal_fg_crf_bg
      semantic_label[(semantic_label_ori >= 1 / 4) & (semantic_label_ori < 2 / 4)] = 2  # sal_bg_crf_fg
      semantic_label[semantic_label_ori < 1 / 4] = 0 # bg
  ...
  def __getitem__(self, idx):
    ...
    if self.depth_paths is not None:
      if self.training:
        image, depth, semantic_label, instance_label, semantic_tag = self._training_preprocess(idx)
      else:
